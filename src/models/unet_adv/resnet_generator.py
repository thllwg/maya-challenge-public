...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, act_fn, norm_layer, stride=1, downsample=None, groups=1,
...
class ResNet(torch.nn.Module):
    ...
    def __init__(self, block, layers, act_fn: nn.Module, norm_layer: nn.Module, c_in: int, groups: int = 1,
        ...
        modules = [
            nn.Sequential(
                ...
                nn.Conv2d(c_in, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False),
            ...
            nn.Sequential(
                ...
                self._make_layer(1, block, 64, layers[0], act_fn, norm_layer),
            ),
            self._make_layer(
                2, block, 128, layers[1], act_fn, norm_layer, stride=2, dilate=replace_stride_with_dilation[0]
            ),
            self._make_layer(
                3, block, 256, layers[2], act_fn, norm_layer, stride=2, dilate=replace_stride_with_dilation[1]
            ),
            self._make_layer(
                4, block, 512, layers[3], act_fn, norm_layer, stride=2, dilate=replace_stride_with_dilation[2]
            )
        ]
